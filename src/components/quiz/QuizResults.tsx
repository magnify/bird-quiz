...
export default function QuizResults({
  ...
  let icon: string
  let tier: 'expert' | 'great' | 'good' | 'practice'
  if (pct >= 90) {
    icon = '\u{1F3C6}'
    tier = 'expert'
  } else if (pct >= 70) {
  ...
  const title = t(`${tier}Title`)
  const subtitle = t(`${tier}Subtitle`)
  ...
  const handleShare = async () => {
    const text = t('shareText', {
      ...
      pct,
      points: points.toLocaleString('da-DK'),
      domain: BRAND.domain,
    })
    if (navigator.share) {
      try {
        ...
        {uniqueMissed.length > 0 && (
          <div className="results-missed">
            <p className="setting-label setting-label--spaced">
              {t('missed', { count: uniqueMissed.length })}
            </p>
            <MissedBirdsCarousel
